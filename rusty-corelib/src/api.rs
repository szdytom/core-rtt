...
use crate::syscall;
use crate::types::{DeviceInfo, Direction, GameInfo, MemoryInfo, PosInfo, SensorTile, UpgradeType};
...
pub fn meminfo() -> Result<MemoryInfo> {
    ...
    // SAFETY: `info` is a valid writable pointer.
    let code = unsafe { syscall::meminfo((&mut info as *mut MemoryInfo).cast()) };
    status(code)?;
    Ok(info)
}
