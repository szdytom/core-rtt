...
std::vector<std::uint8_t> loadBinary(std::string_view path) {
	...
	std::ifstream file(path_string, std::ios::binary);
	if (!file) {
		throw std::runtime_error(
			std::format("Failed to open ELF file: {}", path_string)
		);
	}
	...
	if (content.size() < 4 || content[0] != 0x7F || content[1] != 'E'
		...
		throw std::runtime_error(
			std::format("File is not a valid ELF binary: {}", path_string)
		);
	}
...
Tilemap loadTilemap(std::string_view path) {
	...
	if (!file) {
		throw std::runtime_error(
			std::format("Failed to open tilemap file: {}", path_string)
		);
	}
